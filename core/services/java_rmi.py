...
def footprint(target):
    report = OUTPUT_DIR / f"{target}_java_rmi_report.md"
    print(f"[+] Generating Java RMI recon report → {report}")

    nmap_java_rmi = _run(["nmap", "-p", "1099", "-sV", "-Pn", target])

    msf_queries = [nmap_java_rmi]
    ...
    metadata = {
        "target": target,
        "nmap_java_rmi_banner": nmap_java_rmi,
    ...
    with report.open("w") as rpt:
        rpt.write("# Java RMI Recon Report\n")
        rpt.write(f"## Target: {target}\n\n")

        rpt.write("## Nmap Java RMI Scan\n```\n")
        rpt.write(nmap_java_rmi + "\n```\n\n")
    ...
    with (OUTPUT_DIR / f"{target}_java_rmi_metadata.json").open("w") as jf:
        json.dump(metadata, jf, indent=2)

    print(f"[✓] Report written: {report}")
    print(f"[✓] Metadata JSON written: {OUTPUT_DIR / f'{target}_java_rmi_metadata.json'}")
...
if __name__ == "__main__":
    import sys
    if len(sys.argv) < 2:
        print("Usage: ./java_rmi.py <target>")
        exit(1)
